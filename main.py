                ...
                if consent in ("y", "j", "o", "s", "e"):
                    ...
                    try:
                        ...
                        if getattr(sys, 'frozen', False):
                            # Running as .exe, use update_exe.py
                            import subprocess
                            updater_path = os.path.join(os.path.dirname(sys.executable), 'update_exe.py')
                            # If updater does not exist, write it from bundled resource or download (here, we write from local file if present)
                            if not os.path.exists(updater_path):
                                local_updater = os.path.join(os.path.dirname(__file__), 'update_exe.py')
                                if os.path.exists(local_updater):
                                    with open(local_updater, 'r', encoding='utf-8') as src, open(updater_path, 'w', encoding='utf-8') as dst:
                                        dst.write(src.read())
                                else:
                                    print('[!] update_exe.py not found. Cannot update.')
                                    input('Press Enter to exit...')
                                    sys.exit(1)
                            print('Launching updater...')
                            subprocess.Popen([sys.executable, updater_path])
                            print('Updater started. Exiting main program for update...')
                            input('Press Enter to exit...')
                            sys.exit(0)
                        else:
                            # .py self-update logic
...
# --- Write version.md in the same directory as the .exe or script ---
def ensure_version_md(version):
    # Always write version.md next to the running script or .exe
    if getattr(sys, 'frozen', False):
        version_dir = os.path.dirname(sys.executable)
    else:
        version_dir = os.path.dirname(os.path.abspath(__file__))
    version_file = os.path.join(version_dir, "version.md")
